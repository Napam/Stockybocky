...
def update_features(n_clicks_feats: int, n_clicks_plots: int, features: list, color: str, size: str, 
    ...
    if (prop_id == 'update-feats-button') or (prop_id is None):
        analyzer.preprocess(features)
        dfx = analyzer.fit_and_score()

        dfx.insert(0, 'sector', analyzer.df_raw.sector_osebx)
        dfx.insert(0, 'ticker', analyzer.df_raw.ticker)
        dfx.insert(0, 'name', analyzer.df_raw.name)
        dfx = dfx.sort_values('score')[::-1]
        dfx = dfx.round(4)
        analyzer.get_representations()

        cache['scorehist'] = analyzer.get_score_hist()
...
def get_graphgrid_html(color, size):
    figs = analyzer.get_plots(color, size)
